export function validateRequired(val: string): string | undefined {
	if (!val.trim()) return "Required";
	return undefined;
}

export function makeStoredLabel(id: string, email: string): string {
	return `git-switch-desktop:${id}:${email}`;
}
